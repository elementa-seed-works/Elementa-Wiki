...
**들여쓰기를 목록 태그가 아니라 공백 문자로 한다.** `<summary>` 의 펼침 삼각형은
`list-style-position: inside` 라서 summary 내용의 맨 앞, 즉 텍스트 안쪽에 그려진다.
그래서 하위가 있는 항목만 제목이 삼각형 폭만큼 밀려 하위가 없는 항목과 왼쪽이 어긋난다.
여기에 `<ul>` 을 쓰면 불릿(`◦`)까지 얹혀 더 어수선해진다. GitHub 은 `style` 속성을 제거하므로
CSS 로 맞출 수도 없다. 그래서 목록 태그 없이 항목을 `<br>` 로 나누고,
**하위가 없는 항목 앞에 삼각형 폭만큼 공백을 넣어** 형제끼리 왼쪽을 맞춘다.

깊이 한 단계와 삼각형 자리 모두 `&emsp;`(1em)를 쓴다. 값은 `lib/sidebar.mjs` 의
`INDENT`, `LEAF_PAD` 상수에 있다. 실제 삼각형 폭은 브라우저·폰트에 따라 조금 달라지므로,
어긋나 보이면 이 두 값만 조절하면 된다.
